...
create table if not exists Faculty (
    SSN char(9) primary key references Person on delete cascade,
    FacSupervisor char(9) references Faculty(SSN),
    FacDept char(6),
    FacRank char(4),
    FacHireDate date,
...
create table if not exists Offering (
    ...
    OffDays char(8),
    OffTime time,
    FacSSN references Faculty(SSN)
);

create table if not exists Enrollment (
    StdSSN char(9) references Student(SSN),
